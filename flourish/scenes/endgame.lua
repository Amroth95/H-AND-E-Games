...
function scene:create( event )
    ...
    local myText = display.newText( "Game is completed:", 270, 200, native.systemFont, 16 )
    myText:setFillColor( 0, 0, 0 )
    sceneGroup:insert( myText )

    -- create return to splashscreen button
    -- create a group
    local group = display.newGroup()
    -- make a rounded ractangle
    local myRoundedRect = display.newRoundedRect( 270, 240, 180, 50, 12 )
    ...
    local myButtonText = display.newText( "Return to Splashscreen", 270, 240, native.systemFont, 16 )
    ...
    function buttonFunction (event)
      ...
      composer.gotoScene( "scenes.splashscreen", options )
    end
